namespace CodeJunkie.Metadata.Generator;
...
/// <summary>
/// Generates metadata for types in the developer's codebase to support
/// polymorphic serialization, deserialization, and automatic state management.
/// <br />
/// For more details, see:
/// - Polymorphism: https://learn.microsoft.com/en-us/dotnet/standard/serialization/system-text-json/polymorphism?pivots=dotnet-8-0#configure-polymorphism-with-the-contract-model
/// - Disabling Reflection: https://learn.microsoft.com/en-us/dotnet/standard/serialization/system-text-json/source-generation?pivots=dotnet-8-0#disable-reflection-defaults
/// - AOT/iOS Environments: https://github.com/dotnet/runtime/issues/31326
/// </summary>
[Generator]
public class TypeGenerator : IIncrementalGenerator {
  public void Initialize(IncrementalGeneratorInitializationContext context) {
    // Uncomment the following line to debug the source generator using Visual Studio 2022 on Windows.
    // Run `dotnet build` in the consuming project to attach the debugger.
    //
    ...
    // For macOS debugging with VSCode, use the command palette to select
    // "Debug: Attach to a .NET 5+ or .NET Core process" and search for "VBCS".
    // Attach to the matching compiler process to begin debugging.
    ...
    var incrementalGenerationData = context.SyntaxProvider.CreateSyntaxProvider(
      ...
      .Select((data, _) => {
          ...
          var uniqueTypes = uniqueTypeList.ToDictionary(type => type.FullNameOpen, type => type);

          var tree = new ScopeTree(uniqueTypeList, uniqueTypes);

          var visibleTypeFullNames = tree.GetTypes().Select(t => t.FullNameOpen);

          var visibleTypesBuilder = ImmutableHashSet.CreateBuilder<DeclaredType>();
    ...
    context.RegisterSourceOutput(
        ...
        action: static (context, registry) => {
  ...
  public static bool OutputMetatypesAndReportDiagnostics(SourceProductionContext context,
    ...
    foreach (var type in registry.AllTypes.Values) {
      ...
      if (type.Kind is DeclaredTypeKind.AbstractType &&
      ...
      var invisibleTypes = type.ValidateTypeAndContainingTypes(
          registry.AllTypes, (t) => t.IsPublicOrInternal).ToArray();
      ...
      var nonPartialTypes = type.ValidateTypeAndContainingTypes(
          registry.AllTypes, (t) => t.Reference.IsPartial).ToArray();
      ...
      var genericTypes = type.ValidateTypeAndContainingTypes(
          registry.AllTypes, (t) => t.Reference.TypeParameters.Length == 0).ToArray();
      ...
      if (!registry.VisibleTypes.Contains(type) ||
      ...
      while (currentType.BaseType is { } baseTypeReference) {
        ...
        if (currentType is not null and not DeclaredType { Kind: DeclaredTypeKind.Interface }) {
  ...
  public static void GenerateTypeRegistry(SourceProductionContext context,
                                          DeclaredTypeRegistry registry) {
    var writer = CreateCodeWriter();
    ...
    context.AddSource(hintName: "TypeRegistry.g.cs", source: writer.InnerWriter.ToString());
  }

  public static DeclaredType ResolveDeclaredTypeInfo(GeneratorSyntaxContext context, CancellationToken _) {
  ...
  public static ImmutableArray<string> GetTypeParameters(TypeDeclarationSyntax typeDecl) =>
  ...
  /// <summary>
  /// Determines if the type declaration is explicitly marked as public or internal.
  /// Note: This does not account for visibility of containing types.
  /// </summary>
  ...
  public static ImmutableHashSet<UsingDirective> GetUsings(TypeDeclarationSyntax type) {
  ...
  public static ImmutableArray<DeclaredProperty> GetProperties(TypeDeclarationSyntax type) {
    ...
    foreach (var property in type.Members.OfType<PropertyDeclarationSyntax>()) {
      ...
      var hasGetter = property.AccessorList?.Accessors
        .Any(accessor => accessor.IsKind(SyntaxKind.GetAccessorDeclaration)) ?? property.ExpressionBody is not null;

      // Never identified a situation in which the accessor list is null.
      var hasSetter = property.AccessorList?.Accessors
        .Any(accessor => accessor.IsKind(SyntaxKind.SetAccessorDeclaration)) ?? false;

      var isInit = property.AccessorList?.Accessors
        .Any(accessor => accessor.IsKind(SyntaxKind.InitAccessorDeclaration)) ?? false;
      ...
      var defaultValueExpression = property.Initializer?.Value.NormalizeWhitespace().ToString();
  ...
  public static ImmutableArray<string> GetMixins(TypeDeclarationSyntax typeDecl) {
    ...
    foreach (var attributeList in typeDecl.AttributeLists) {
      foreach (var attr in attributeList.Attributes) {
        if (attr.Name.ToString() == Constants.MetadataAttributeName) {
          mixins.AddRange(
  ...
  public static ImmutableArray<DeclaredAttribute> GetAttributes(SyntaxList<AttributeListSyntax> attributeLists) {
    ...
    foreach (var attr in attributeLists) {
      foreach (var arg in attr.Attributes) {
        ...
        foreach (var argExpr in arg.ArgumentList?.Arguments ??
  ...
  public static IndentedTextWriter CreateCodeWriter() => new(new StringWriter(), "  ");
}
