...
public sealed record TypeLocation(ImmutableArray<string> Namespaces,
  ...
  /// <summary>Full name of the type that represents this location. </summary>
  public string FullNameOpen => string.Join(
    ".",
    Namespaces.Concat(ContainingTypes.Select(t => t.SimpleNameOpen)));

  /// <summary>True if the location is within a generic type. </summary>
  public bool IsInGenericType => ContainingTypes.Any(t => t.IsGeneric);

  /// <summary>True if the location is not in a nested type, or nested only within only partial types. </summary>
